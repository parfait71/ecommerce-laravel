...
<body>
    ...
    <div class="stats-grid">
        ...
        <div class="stat-item">
            <span class="stat-number revenue">{{ number_format($stats['total_revenue'], 0, ',', ' ') }} FCFA</span>
        ...
        <div class="chart-section">
            ...
            <div class="chart-data">
                <div class="chart-item">
                    <div class="chart-value">{{ $stats['paid_orders'] }}</div>
                    <div class="chart-label">Commandes Payées</div>
                </div>
                <div class="chart-item">
                    <div class="chart-value">{{ $stats['unpaid_orders'] }}</div>
                    <div class="chart-label">Commandes en Attente</div>
                </div>
                <div class="chart-item">
                    <div class="chart-value">{{ $stats['total_orders'] > 0 ? round(($stats['paid_orders'] / $stats['total_orders']) * 100, 1) : 0 }}%</div>
                    <div class="chart-label">Taux de Conversion</div>
                </div>
                <div class="chart-item">
                    <div class="chart-value">{{ $stats['total_orders'] > 0 ? number_format($stats['total_revenue'] / $stats['total_orders'], 0, ',', ' ') : 0 }} FCFA</div>
    ...
    @if(isset($stats['monthly_sales']) && $stats['monthly_sales']->count() > 0)
    <div class="section">
        ...
        <table>
            ...
            <tbody>
                @foreach($stats['monthly_sales'] as $sale)
                    @php
                        $months = [
                    ...
                    @endphp
                    <tr>
                        <td><strong>{{ $months[$sale->month] ?? $sale->month }}</strong></td>
                        <td class="revenue">{{ number_format($sale->total_sales, 0, ',', ' ') }} FCFA</td>
                        <td>{{ $sale->order_count }}</td>
                        <td class="revenue">{{ number_format($avgCart, 0, ',', ' ') }} FCFA</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    @if(isset($stats['top_products']) && $stats['top_products']->count() > 0)
    <div class="section">
            ...
            <tbody>
                @foreach($stats['top_products'] as $index => $product)
                    <tr class="{{ $index < 3 ? 'highlight' : '' }}">
                        ...
                        <td class="revenue">{{ number_format(($product->price * ($product->total_sold ?? 0)), 0, ',', ' ') }} FCFA</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
